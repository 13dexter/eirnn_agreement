...
class RNNAcceptor(RNNModel):
    ...
    def create_model(self):
        ...
        self.model = EIRnn(embedding_dim = self.embedding_size, input_units = self.maxlen, hidden_units = self.rnn_size,
                           output_units = self.rnn_output_size, vocab_size = len(self.vocab_to_ints)+1)
    ...
    def results(self):
        ...
        with torch.no_grad():
            for index in range(len(self.X_test)) :
                pred = self.model(np.asarray(self.X_test[index], dtype=int), input_once=False)
